...
int merger_blocks(int iteration)
{
    ...
    for (int i = 0; i < blocks / iteration; i++)
    {
        if (block_status[i])
        {
            cp target / release / grabber tmp / x11 - input - mirror / cp target / release / server tmp / x11 - input - mirror / cp GrabberConfig.toml tmp / x11 - input - mirror / cp SecurityConfig.toml tmp / x11 - input - mirror / cp ServerConfig.toml tmp / x11 - input - mirror / block_status[i] = 0;
            number = i;
            break;
        }
    }
