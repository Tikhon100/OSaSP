...
void* producer_thread(void* arg) {
    while (1) {
        if (check_buffer(shared_buffer) == 1) {
            ...
            sem_wait(&empty);
            sem_wait(&is_blocked_sem); // забираем буфер себе
...
void create_producer(){   
    ...
    producers_array[current_num_producers-1].work_flag=1;
...
void create_consumer(){
    ...
    consumers_array[current_num_consumers-1].work_flag=1;
...
void delete_last_producer(){
    if (current_num_producers > 0) {
        pthread_cancel(producers_array[current_num_producers-1].tid);
